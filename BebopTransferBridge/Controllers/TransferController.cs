...
[ApiController]
[Route("[controller]")]
public class TransferController(IMessageSender messageSender, IOptions<BebopTransferQueues> exchangeOptions, ILogger<TransferController> logger) : ControllerBase
{
    ...
    {
        logger.LogInformation("new message from transfer from external operator with userId {id}", userId);
